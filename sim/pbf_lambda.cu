...
namespace {
        ...
        uint32_t N)
    {
        ...
        for (int dx = -reach; dx <= reach; ++dx) {
            if (hasCap && neighborContrib >= cap) goto DoneDense;
            ...
            for (uint32_t k = beg; k < end; ++k) {
                ...
                float hr2 = kc.h2 - r2;
                density += particleMass * kc.poly6 * hr2 * hr2 * hr2;

                float r = sqrtf(r2);
                float r_safe = fmaxf(r, pbf.grad_r_eps);
                float t = kc.h - r;
                float coeff = (-3.0f * kc.spiky * (t * t) / r_safe);
    ...
    DoneDense:
        float gradSum2 = gradSum.x * gradSum.x + gradSum.y * gradSum.y + gradSum.z * gradSum.z;
        float scale = (restDensity > 0.f) ? (particleMass / restDensity) : 0.f;
        ...
        float C = (restDensity > 0.f) ? (density / restDensity - 1.f) : 0.f;
        float lam = -C / denom;
        if (!isfinite(lam)) lam = 0.f;
        ...
        uint32_t N)
    {
        ...
        for (int dy = -reach; dy <= reach; ++dy)
        for (int dx = -reach; dx <= reach; ++dx) {
            if (hasCap && neighborContrib >= cap) goto DoneCompact;
            int3 cc = make_int3(ci.x + dx, ci.y + dy, ci.z + dz);
            if (!sim::inBounds(cc, grid.dim)) continue;
            uint32_t cidx = sim::linIdx(cc, grid.dim);
