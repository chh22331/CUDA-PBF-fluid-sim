...
// =====================================================================
// D2D Memcpy Logging Utilities
// =====================================================================
// Unified logging for device-to-device memory copies.
// Respects console.debug.printDiagnostics and console.debug.printWarnings flags.
// Search tag: [D2D-Memcpy]
...
namespace sim {
    ...
    // Utility: check if host pointer is readable (Windows-specific).
...
// D2D memcpy macros with logging support.
...
namespace sim {
    ...
    // =====================================================================
    // Ping-Pong Position Buffer Management
    // =====================================================================
    ...
    void Simulator::performPingPongSwap(uint32_t N) {
        ...
        // Patch compute graph if in use.
        if (m_graphExec) {
            patchGraphPositionPointers(oldCurr, oldNext);
        }
    }

    // Patch CUDA graph kernel nodes with updated position buffer pointers.
    void Simulator::patchGraphPositionPointers(float4* oldCurr, float4* oldNext) {
        ...
        for (auto nd : m_posNodes) {
            ...
            // Scan parameter array for position buffer pointers.
    ...
    bool Simulator::initialize(const SimParams& p) {
        ...
        // Initialize context and grid strategy.
        m_ctx.bufs = &m_bufs;
        m_ctx.grid = &m_grid;
        ...
        // Allocate buffers and grid structures.
        ...
        // Initialize curr -> next -> pred positions (pred aliases next for predictive scheme).
        if (p.numParticles > 0) {
        ...
        // Initialize state flags and pipelines.
        ...
        // Build default simulation pipelines.
    ...
    void Simulator::shutdown() {
        // Release external D3D12 memory bindings.
        if (m_extPosPred) {
        ...
        // Release grid and graph resources.
        m_grid.releaseAll();
        if (m_graphExec) {
        ...
        for (int i = 0; i < 2; ++i) {
            if (m_evStart[i]) {
                cudaEventDestroy(m_evStart[i]);
                m_evStart[i] = nullptr;
            }
            if (m_evEnd[i]) {
                cudaEventDestroy(m_evEnd[i]);
                m_evEnd[i] = nullptr;
            }
        }
    ...
    bool Simulator::updateGridIfNeeded(const SimParams& p) {
        ...
        m_params.grid = p.grid;
        m_params.grid.dim = dim;
        m_grid.ensureCompactCapacity(
            p.maxParticles > 0 ? p.maxParticles : p.numParticles
        );
        return changed;
    }
    ...
    bool Simulator::cacheGraphNodes() {
        ...
        // Classify nodes based on watched buffers (velocity vs. position).
        for (auto nd : nodes) {
            cudaGraphNodeType t;
            ...
            if (hasVel) {
                m_velNodes.push_back(nd);
                m_velNodeParamsBase.push_back(kp);
            }
            if (hasPos) {
                m_posNodes.push_back(nd);
                m_posNodeParamsBase.push_back(kp);
            }
        }
    ...
    bool Simulator::paramOnlyGraphChange(const SimParams& p) const {
        ...
        // Quick check for negligible changes.
        if (dtRel < 0.002f &&
    ...
    bool Simulator::captureGraphIfNeeded(const SimParams& p) {
        ...
        if (!m_graphPointersChecked) {
            if (!m_bufs.d_pos || !m_bufs.d_vel || !m_bufs.d_pos_pred) {
                std::fprintf(stderr,
                    "[Graph][Error] Required buffers null before capture "
                    "(pos=%p vel=%p pos_pred=%p)\n",
    ...
    bool Simulator::step(const SimParams& p) {
        ...
        // Update frame timing configuration.
        ...
        // Clamp and validate particle capacity.
        ...
        // Reallocate buffers if particle count increased.
        ...
        // Detect parameter changes and mark graph dirty if needed.
        ...
        // Determine if we should collect frame timing data.
    ...
    // =====================================================================
    // External Memory Bindings (D3D12)
    // =====================================================================
    ...
    bool Simulator::bindExternalPosPingPong(void* sharedHandleA, size_t bytesA,
        ...
        // Ensure CUDA context is active before importing D3D12 resources.
        cudaFree(0);
    ...
    bool Simulator::bindExternalVelocityBuffer(void* sharedHandleWin32, size_t bytes,
    ...
    void Simulator::seedBoxLattice(uint32_t nx, uint32_t ny, uint32_t nz,
        ...
        // Generate lattice positions on host.
        ...
        if (c.sim.initial_jitter_enable) {
            ...
            if (amp > 0.f) {
                ...
                for (uint32_t i = 0; i < N; ++i) {
                    float ox, oy, oz;
                    // Sample random point in unit sphere.
                    for (;;) {
                        ox = J(jrng);
                        oy = J(jrng);
                        oz = J(jrng);
    ...
    void Simulator::seedBoxLatticeAuto(uint32_t total, float3 origin, float spacing) {
        ...
        // Compute lattice dimensions that fit the domain.
        ...
        // Ensure minimum coverage while staying within bounds.
        ...
        if (Nreq < total) {
            ...
            if (c.debug.printWarnings) {
                std::fprintf(stderr,
                    "[Warn] seedBoxLatticeAuto: Nreq(%u) < total(%u). Potential overlap.\n",
                    Nreq, total);
            }
        }

        // Clamp origin with margin.
        float3 margin = make_float3(spacing * 0.25f, spacing * 0.25f, spacing * 0.25f);
        origin.x = fminf(fmaxf(origin.x, m_params.grid.mins.x + margin.x),
                         m_params.grid.maxs.x - margin.x);
        origin.y = fminf(fmaxf(origin.y, m_params.grid.mins.y + margin.y),
                         m_params.grid.maxs.y - margin.y);
        origin.z = fminf(fmaxf(origin.z, m_params.grid.mins.z + margin.z),
                         m_params.grid.maxs.z - margin.z);
    ...
    void Simulator::seedCubeMix(uint32_t groupCount, const float3* centers, uint32_t edgeParticles,
        ...
        // Generate cube groups around each center.
        uint32_t cursor = 0;
    ...
    // =====================================================================
    // Synchronization & External Fence Management
    // =====================================================================
    ...
    void Simulator::publishExternalVelocity(uint32_t count) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(m_extVelocityPtr, m_bufs.d_vel, bytes,
    ...
    // =====================================================================
    // CUDA Graph Velocity Buffer Pointer Patching
    // =====================================================================
