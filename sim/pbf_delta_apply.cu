...
namespace {
        ...
        uint32_t N)
    {
        ...
        uint32_t i = indicesSorted[sortedIdx];
        float4 di4 = delta[i];
        float4 base = pos_pred_fp32[i];
        pos_pred_fp32[i] = make_float4(base.x + di4.x, base.y + di4.y, base.z + di4.z, base.w);
    }
    ...
    cudaStream_t s)
{
    ...
    KDeltaCompute<<<gridDim, block, 0, s>>>(
        delta, pos_pred, lambda, indicesSorted, keysSorted, cellStart, cellEnd, dp, N);
    ...
    cudaStream_t s)
{
    ...
    KDeltaComputeCompact<<<gridDim, block, 0, s>>>(
        delta, pos_pred, lambda, indicesSorted, keysSorted, uniqueKeys, offsets, compactCount, dp, N);
    KApplyDelta<<<gridDim, block, 0, s>>>(pos_pred, delta, indicesSorted, N);
}