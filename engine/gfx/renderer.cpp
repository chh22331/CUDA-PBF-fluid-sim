...
namespace gfx {
    namespace {
        constexpr uint32_t kAudioDebugCapacityDefault = 128;
        struct Mat4 { float m[16]; };
        static Mat4 mul(const Mat4& a, const Mat4& b) {
            Mat4 r{}; for (int c = 0; c < 4; ++c) for (int r0 = 0; r0 < 4; ++r0) r.m[c * 4 + r0] =
        ...
        static Microsoft::WRL::ComPtr<ID3D12RootSignature> CreateRootSignatureGfx(ID3D12Device* dev) {
            ...
            D3D12_DESCRIPTOR_RANGE1 rangeAudio{};
            rangeAudio.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
            rangeAudio.NumDescriptors = 1;
            rangeAudio.BaseShaderRegister = 2; // t2
            rangeAudio.RegisterSpace = 0;
            rangeAudio.Flags = D3D12_DESCRIPTOR_RANGE_FLAG_NONE;
            rangeAudio.OffsetInDescriptorsFromTableStart = 0;
            ...
            // t2 audio intensities
            params[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
            params[3].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
            D3D12_ROOT_DESCRIPTOR_TABLE1 tbl2{};
            tbl2.NumDescriptorRanges = 1;
            tbl2.pDescriptorRanges = &rangeAudio;
            params[3].DescriptorTable = tbl2;
        ...
        D3D12_BLEND_DESC AlphaBlendDesc() {
            D3D12_BLEND_DESC b{}; b.AlphaToCoverageEnable = FALSE; b.IndependentBlendEnable = FALSE;
        ...
        D3D12_RASTERIZER_DESC DefaultRasterizerNoCull() {
            D3D12_RASTERIZER_DESC r{}; r.FillMode = D3D12_FILL_MODE_SOLID; r.CullMode = D3D12_CULL_MODE_NONE;
    ...
    bool RendererD3D12::Initialize(HWND hwnd, const RenderInitParams& p) {
        ...
        EnsureAudioDebugBuffer(kAudioDebugCapacityDefault);
    ...
    void RendererD3D12::Shutdown() {
        ...
        if (m_audioDebugBuffer && m_audioDebugCpuPtr) {
            D3D12_RANGE range{ 0, 0 };
            m_audioDebugBuffer->Unmap(0, &range);
            m_audioDebugCpuPtr = nullptr;
        }
        m_audioDebugBuffer.Reset();
        m_audioDebugSrvIndex = -1;
        m_audioDebugKeyCapacity = 0;
    ...
    void RendererD3D12::BuildFrameGraph() {
        ...
        if (rsGfx && m_psoPointsFloat) {
            ...
            points.execute = [this, rsGfx]() {
                ...
                cb.audioDebug0 = make_float4(m_audioDebug.enabled, m_audioDebug.domainMinX, m_audioDebug.invDomainWidth, 0.0f);
                uint32_t debugKeyCount = std::min(m_audioDebug.keyCount, m_audioDebugKeyCapacity);
                float audioInvKeys = (debugKeyCount > 0) ? (1.0f / float(debugKeyCount)) : 0.0f;
                cb.audioDebug1 = make_float4(m_audioDebug.surfaceY, m_audioDebug.surfaceFalloff, float(debugKeyCount), audioInvKeys);
                ...
                if (m_audioDebugSrvIndex >= 0) {
                    auto gpuAudio = m_device.srvGpuHandleAt((uint32_t)m_audioDebugSrvIndex);
                    cl->SetGraphicsRootDescriptorTable(3, gpuAudio);
                }
    ...
    bool RendererD3D12::EnsureAudioDebugBuffer(uint32_t minCapacity) {
        uint32_t desired = (minCapacity == 0) ? 1u : minCapacity;
        if (m_audioDebugBuffer && m_audioDebugKeyCapacity >= desired && m_audioDebugCpuPtr) {
            return true;
        }
        if (m_audioDebugBuffer && m_audioDebugCpuPtr) {
            D3D12_RANGE range{ 0, 0 };
        ...
        D3D12_RESOURCE_DESC rd{}; rd.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
        rd.Width = uint64_t(sizeof(float)) * uint64_t(desired);
        rd.Height = 1; rd.DepthOrArraySize = 1; rd.MipLevels = 1; rd.Format = DXGI_FORMAT_UNKNOWN;
        rd.SampleDesc = { 1,0 }; rd.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR; rd.Flags = D3D12_RESOURCE_FLAG_NONE;
        if (FAILED(m_device.device()->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd,
            D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_audioDebugBuffer)))) {
            return false;
        }
        if (FAILED(m_audioDebugBuffer->Map(0, nullptr, reinterpret_cast<void**>(&m_audioDebugCpuPtr)))) {
            m_audioDebugBuffer.Reset();
            return false;
        }
        std::memset(m_audioDebugCpuPtr, 0, sizeof(float) * desired);
        m_audioDebugKeyCapacity = desired;
        m_audioDebugSrvIndex = m_device.createBufferSRV(m_audioDebugBuffer.Get(), desired, sizeof(float));
        return m_audioDebugSrvIndex >= 0;
    }
    void RendererD3D12::SetAudioDebugIntensities(const float* values, uint32_t keyCount) {
        uint32_t desired = (keyCount == 0) ? 1u : keyCount;
        if (!EnsureAudioDebugBuffer(desired) || !m_audioDebugCpuPtr) {
            return;
        }
        uint32_t copy = (values && keyCount > 0) ? std::min(keyCount, m_audioDebugKeyCapacity) : 0u;
        if (copy > 0) {
            std::memcpy(m_audioDebugCpuPtr, values, sizeof(float) * copy);
        }
        if (copy < m_audioDebugKeyCapacity) {
            std::memset(m_audioDebugCpuPtr + copy, 0, sizeof(float) * (m_audioDebugKeyCapacity - copy));
        }
    }
} // namespace gfx
